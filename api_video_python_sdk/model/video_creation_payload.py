# coding: utf-8

"""
    api.video

    api.video is an API that encodes on the go to facilitate immediate playback, enhancing viewer streaming experiences across multiple devices and platforms. You can stream live or on-demand online videos within minutes.

    The version of the OpenAPI document: 1
    Generated by: https://konfigthis.com
"""

from datetime import date, datetime  # noqa: F401
import decimal  # noqa: F401
import functools  # noqa: F401
import io  # noqa: F401
import re  # noqa: F401
import typing  # noqa: F401
import typing_extensions  # noqa: F401
import uuid  # noqa: F401

import frozendict  # noqa: F401

from api_video_python_sdk import schemas  # noqa: F401


class VideoCreationPayload(
    schemas.DictSchema
):
    """
    This class is auto generated by Konfig (https://konfigthis.com)
    """


    class MetaOapg:
        required = {
            "title",
        }
        
        class properties:
            title = schemas.StrSchema
        
            @staticmethod
            def tags() -> typing.Type['VideoCreationPayloadTags']:
                return VideoCreationPayloadTags
            description = schemas.StrSchema
            source = schemas.StrSchema
            public = schemas.BoolSchema
            panoramic = schemas.BoolSchema
            mp4Support = schemas.BoolSchema
            playerId = schemas.StrSchema
            
            
            class metadata(
                schemas.ListSchema
            ):
            
            
                class MetaOapg:
                    
                    @staticmethod
                    def items() -> typing.Type['Metadata']:
                        return Metadata
            
                def __new__(
                    cls,
                    arg: typing.Union[typing.Tuple['Metadata'], typing.List['Metadata']],
                    _configuration: typing.Optional[schemas.Configuration] = None,
                ) -> 'metadata':
                    return super().__new__(
                        cls,
                        arg,
                        _configuration=_configuration,
                    )
            
                def __getitem__(self, i: int) -> 'Metadata':
                    return super().__getitem__(i)
        
            @staticmethod
            def clip() -> typing.Type['VideoClip']:
                return VideoClip
        
            @staticmethod
            def watermark() -> typing.Type['VideoWatermark']:
                return VideoWatermark
            __annotations__ = {
                "title": title,
                "tags": tags,
                "description": description,
                "source": source,
                "public": public,
                "panoramic": panoramic,
                "mp4Support": mp4Support,
                "playerId": playerId,
                "metadata": metadata,
                "clip": clip,
                "watermark": watermark,
            }
    
    title: MetaOapg.properties.title
    
    @typing.overload
    def __getitem__(self, name: typing_extensions.Literal["title"]) -> MetaOapg.properties.title: ...
    
    @typing.overload
    def __getitem__(self, name: typing_extensions.Literal["tags"]) -> 'VideoCreationPayloadTags': ...
    
    @typing.overload
    def __getitem__(self, name: typing_extensions.Literal["description"]) -> MetaOapg.properties.description: ...
    
    @typing.overload
    def __getitem__(self, name: typing_extensions.Literal["source"]) -> MetaOapg.properties.source: ...
    
    @typing.overload
    def __getitem__(self, name: typing_extensions.Literal["public"]) -> MetaOapg.properties.public: ...
    
    @typing.overload
    def __getitem__(self, name: typing_extensions.Literal["panoramic"]) -> MetaOapg.properties.panoramic: ...
    
    @typing.overload
    def __getitem__(self, name: typing_extensions.Literal["mp4Support"]) -> MetaOapg.properties.mp4Support: ...
    
    @typing.overload
    def __getitem__(self, name: typing_extensions.Literal["playerId"]) -> MetaOapg.properties.playerId: ...
    
    @typing.overload
    def __getitem__(self, name: typing_extensions.Literal["metadata"]) -> MetaOapg.properties.metadata: ...
    
    @typing.overload
    def __getitem__(self, name: typing_extensions.Literal["clip"]) -> 'VideoClip': ...
    
    @typing.overload
    def __getitem__(self, name: typing_extensions.Literal["watermark"]) -> 'VideoWatermark': ...
    
    @typing.overload
    def __getitem__(self, name: str) -> schemas.UnsetAnyTypeSchema: ...
    
    def __getitem__(self, name: typing.Union[typing_extensions.Literal["title", "tags", "description", "source", "public", "panoramic", "mp4Support", "playerId", "metadata", "clip", "watermark", ], str]):
        # dict_instance[name] accessor
        return super().__getitem__(name)
    
    
    @typing.overload
    def get_item_oapg(self, name: typing_extensions.Literal["title"]) -> MetaOapg.properties.title: ...
    
    @typing.overload
    def get_item_oapg(self, name: typing_extensions.Literal["tags"]) -> typing.Union['VideoCreationPayloadTags', schemas.Unset]: ...
    
    @typing.overload
    def get_item_oapg(self, name: typing_extensions.Literal["description"]) -> typing.Union[MetaOapg.properties.description, schemas.Unset]: ...
    
    @typing.overload
    def get_item_oapg(self, name: typing_extensions.Literal["source"]) -> typing.Union[MetaOapg.properties.source, schemas.Unset]: ...
    
    @typing.overload
    def get_item_oapg(self, name: typing_extensions.Literal["public"]) -> typing.Union[MetaOapg.properties.public, schemas.Unset]: ...
    
    @typing.overload
    def get_item_oapg(self, name: typing_extensions.Literal["panoramic"]) -> typing.Union[MetaOapg.properties.panoramic, schemas.Unset]: ...
    
    @typing.overload
    def get_item_oapg(self, name: typing_extensions.Literal["mp4Support"]) -> typing.Union[MetaOapg.properties.mp4Support, schemas.Unset]: ...
    
    @typing.overload
    def get_item_oapg(self, name: typing_extensions.Literal["playerId"]) -> typing.Union[MetaOapg.properties.playerId, schemas.Unset]: ...
    
    @typing.overload
    def get_item_oapg(self, name: typing_extensions.Literal["metadata"]) -> typing.Union[MetaOapg.properties.metadata, schemas.Unset]: ...
    
    @typing.overload
    def get_item_oapg(self, name: typing_extensions.Literal["clip"]) -> typing.Union['VideoClip', schemas.Unset]: ...
    
    @typing.overload
    def get_item_oapg(self, name: typing_extensions.Literal["watermark"]) -> typing.Union['VideoWatermark', schemas.Unset]: ...
    
    @typing.overload
    def get_item_oapg(self, name: str) -> typing.Union[schemas.UnsetAnyTypeSchema, schemas.Unset]: ...
    
    def get_item_oapg(self, name: typing.Union[typing_extensions.Literal["title", "tags", "description", "source", "public", "panoramic", "mp4Support", "playerId", "metadata", "clip", "watermark", ], str]):
        return super().get_item_oapg(name)
    

    def __new__(
        cls,
        *args: typing.Union[dict, frozendict.frozendict, ],
        title: typing.Union[MetaOapg.properties.title, str, ],
        tags: typing.Union['VideoCreationPayloadTags', schemas.Unset] = schemas.unset,
        description: typing.Union[MetaOapg.properties.description, str, schemas.Unset] = schemas.unset,
        source: typing.Union[MetaOapg.properties.source, str, schemas.Unset] = schemas.unset,
        public: typing.Union[MetaOapg.properties.public, bool, schemas.Unset] = schemas.unset,
        panoramic: typing.Union[MetaOapg.properties.panoramic, bool, schemas.Unset] = schemas.unset,
        mp4Support: typing.Union[MetaOapg.properties.mp4Support, bool, schemas.Unset] = schemas.unset,
        playerId: typing.Union[MetaOapg.properties.playerId, str, schemas.Unset] = schemas.unset,
        metadata: typing.Union[MetaOapg.properties.metadata, list, tuple, schemas.Unset] = schemas.unset,
        clip: typing.Union['VideoClip', schemas.Unset] = schemas.unset,
        watermark: typing.Union['VideoWatermark', schemas.Unset] = schemas.unset,
        _configuration: typing.Optional[schemas.Configuration] = None,
        **kwargs: typing.Union[schemas.AnyTypeSchema, dict, frozendict.frozendict, str, date, datetime, uuid.UUID, int, float, decimal.Decimal, None, list, tuple, bytes],
    ) -> 'VideoCreationPayload':
        return super().__new__(
            cls,
            *args,
            title=title,
            tags=tags,
            description=description,
            source=source,
            public=public,
            panoramic=panoramic,
            mp4Support=mp4Support,
            playerId=playerId,
            metadata=metadata,
            clip=clip,
            watermark=watermark,
            _configuration=_configuration,
            **kwargs,
        )

from api_video_python_sdk.model.metadata import Metadata
from api_video_python_sdk.model.video_clip import VideoClip
from api_video_python_sdk.model.video_creation_payload_tags import VideoCreationPayloadTags
from api_video_python_sdk.model.video_watermark import VideoWatermark
